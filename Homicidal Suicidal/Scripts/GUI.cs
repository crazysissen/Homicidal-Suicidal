...
{
    /// <summary>
    /// Universal serializable class for rendering various things to the screen
    /// </summary>
    public class GUI
    {
        ...
        public struct Label : IGUIMember
        {
            ...
            {
                spriteBatch.DrawString(font, text, position, color, 0, Vector2.Zero, (float)fontSize / fontSizeComparator, SpriteEffects.None, 0.9f + 0.0001f * order);
            }
        }
        ...
        public struct Texture : IGUIMember
        {
            ...
            {
                this.rectangle = rectangle;
                this.texture = Game1.AllSprites["Square"];
                this.color = color;
            }
        ...
        public struct Button : IGUIMember
        {
            ...
            GUIButtonState GetState()
            {
                ...
                if (state.Position.X >= rectangle.Left && state.Position.X <= rectangle.Right && state.Position.Y >= rectangle.Top && state.Position.Y <= rectangle.Bottom)
                    return Methods.LeftMouseDown() ? GUIButtonState.Click : GUIButtonState.Hover;
